...
*/
void draw_newgame_team_2(pmenudata data)
{
  ...
  glColor4f(1,0,0,0.5);
  ZWdrawsprite(maintex,-1.7, 0.1,-4,0.8,0.8,2,2);
  glColor4f(0,1,0,0.5);
...
int calc_newgame_team_2(pmenudata data)
{
  //Rotation
    ...
    if (data->choose_step[a]==0)
    {
      ...
      if (*(ZWattribute->joystick[a].button[0]))
      {
         *(ZWattribute->joystick[a].button[0])=0;
         if (distance(-1.7, 0.1,0,data->x[a],data->y[a],0)<=0.8)
         {
           data->choose_step[a]=1;
           maindata.player[a].team=0;
         }
         if (distance(-0.2,-1.0,0,data->x[a],data->y[a],0)<=0.8)
         {
           data->choose_step[a]=1;
  ...
  if (a==playernum || data->cancel_all>playernum/2) //Schleife lief komplett durch oder cancel_all!
  {
    ...
    else
    {
      ...
      if (a==2) //Schleife lief durch
      {
        ...
        float teamcount[2]={0,0};
        for (a=0;a<playernum+cpuplayer;a++)
        {
          switch (maindata.player[a].team)
          {
            case 0: data->x[a]=-1.7+cos(teamcount[maindata.player[a].team]*M_PI*2.0/((float)teamsize))*0.3;
                    data->y[a]= 0.1+sin(teamcount[maindata.player[a].team]*M_PI*2.0/((float)teamsize))*0.3;
            break;
            case 1: data->x[a]=-0.2+cos(teamcount[maindata.player[a].team]*M_PI*2.0/((float)teamsize))*0.3;
                    data->y[a]=-1.0+sin(teamcount[maindata.player[a].team]*M_PI*2.0/((float)teamsize))*0.3;
            break;
          }
...
int calc_newgame_team_2_thread(pmenudata data)
{
